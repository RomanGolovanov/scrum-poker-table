(function(){
  ...
  angular
    ...
    .controller("PlayDeskHistoryController", ["$scope", "$routeParams", "$location", "$timeout", "DeskService",
    function($scope, $routeParams, $location, $timeout, deskService){

        $scope.desk_id = $routeParams.desk_id;
        $scope.desk_url = $location.absUrl();

        deskService.get_history($scope.desk_id).then(function(desk_history){
            $scope.desk_history = desk_history.map(function(h){
